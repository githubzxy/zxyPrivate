...
<div class="rightinfo">   
  <div class="formbody">
     ...
     <form class="registerform" id = "insertForm">
     	<input type="hidden" name="themeId" value="${discussThemePo.themeId}"/>
            	...
            	<td class="td_right">所属课程：</td>
                 <td> 
                     <select  name="openCourseId" id="openCourseId"  class="select" style="width:250px;">
                         ...
                         <option value="${openCourseVos.openId}" <c:if test="${discussThemePo.openCoursePo.openId==openCourseVos.openId}">
                         	selected="selected"
                         </c:if>
                         >${openCourseVos.courseName}</option>
                         </c:forEach>
                     </select>
		                 ...
		                 <tr>
		                  ...
		                  <td class="">
		                    <textarea name="title" id="title" cols="80" rows="10" class="textarea">${discussThemePo.title}</textarea>
		                  </td>
		                 </tr>
	...
	var openCourseId = $("#openCourseId").val();
	 var title=$("#title").val();
		...
		if(title != null && title !=""){
			...
			$.ajax({
				  type: "POST",
				  url: "${res}/admin/discuss/updataTheme.html",
				  processData:true,
				  data:{"openId":openCourseId,"title":title,themeId:$("input[name='themeId']").val()},
				  dataType: "json",
