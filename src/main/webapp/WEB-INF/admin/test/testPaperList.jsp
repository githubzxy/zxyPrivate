...
//抽取公共部分1
function PageTmp(title,size,page){
	$.ajax({
		  type: "POST",
		  url: "${res}/test/getPaperByPaperIdsOfTest.html",
		  processData:true,
			  ...
			  if((typeof(size) == "undefined")){
				  ...
				  $("#pageToolbar").empty();
					  $('#pageToolbar').Paging({pagesize:data.page.pageSize,count:data.page.totalPage,toolbar:true,callback:function(page,size,count){
							PageTmp(title,size,page);
						  }}); 
			  }
			HtmlTmp(data.page.list,data.page.startIndex);
			  }else{
				  $testStr.append("<tr><td colspan='9' style='text-align:center;color:red;'>暂无|･ω･｀)</td></tr>");
			  }
		  },
		  error:function(e){
			  alert("错误");
...
function HtmlTmp(datas,startIndex){
	 $testStr.empty();
	  $.each(datas, function (index, test) {
		...
		if(test.subjectNum==null|test.subjectNum==0){
			htmlStr+='<td>无</td>';
		}else{
			htmlStr+='<td>'+test.subjectNum+'</td>';
		}
		htmlStr+='<td>';
		htmlStr+='<span class="pl25 ml15">';
		htmlStr+='<input class="ext_btn ext_btn_success"onclick="www_helpor_net('+test.paperId+');" type="button" value="查看"/>';
		htmlStr+='</span> '; 
		htmlStr+='</td> '; 
		htmlStr+='</tr>'; 
...
function   www_helpor_net(paperId){  
	  var   targeturl="${res }/test/checkPaperOfTest.html?paperId="+paperId; 
