...
<body>
    ...
    <div class="tools">
       ...
       <ul class="prosearch">
           <li>
               <i>资源名称:</i><a><input name="name" type="text" class="input-text code" placeholder="资源关键字" value="${page.code }"/></a>
                ...
                <td>
                    <span class="pl15">
                        ...
                        <input class="ext_btn ext_btn_error mr20" value="删除" type="button"  onclick="del(${resource.id})" /><!--如果不是管理员登录就不显示删除按钮-->
                         <c:if test="${resource.state != 2 && resource.state != 3}">
                       	  	<input class="ext_btn ext_btn_success" value="分享资源" type="button" onclick="share(this,${resource.id})" data-type="-1"/><!--如果不是管理员登录就不显示删除按钮-->
                         </c:if>
                         <c:if test="${resource.state == 2 || resource.state == 3}">
 ...
 function PageTmp(name,size,page){
 	...
 	$.ajax({
		  type: "POST",
		  url: "${res}/resource/myPublicList.html",
		  processData:true,
       		...
       		success:function(data){
       			if(1 == data.flag){
					  layer.alert(data.msg,{icon:1},function(){
	      				  document.location.href=" ${res}/resource/myPublicList.html";
	      			  });
