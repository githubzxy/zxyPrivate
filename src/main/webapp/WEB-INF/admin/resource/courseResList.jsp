      ...
      <thead>
        ...
        <tbody id="courseResStr">
          <c:forEach  items="${resourceList }" var="resource" varStatus="status">
            <tr>
                <td><input name="ur_checkbox" class="resourceId" value="${resource.id}" type="checkbox"/></td>
                ...
                <c:if test="${resource.state != 3 }">
                	<td class="f00 stauts">未审核</td>
                </c:if>
                <c:if test="${resource.state == 3 }">
                	<td class="green stauts">已审核</td>
                </c:if>
                <td>
                    <span class="pl15">
                        <a href="javaScript:;" url="${res }/upload/downFile.html?fn=${resource.url}" id="start-btn" class="ext_btn ext_btn_success mr20">下载</a>
                        <input class="ext_btn ext_btn_error mr20" value="删除" type="button"  onclick="del(${resource.id})" /><!--如果不是管理员登录就不显示删除按钮-->
                         <c:if test="${resource.state == 3 }">
                       	  	<input class="ext_btn ext_btn_success" value="已审核" type="button" onclick="audit(this,${resource.id})" data-type="-1"/><!--如果不是管理员登录就不显示删除按钮-->
                         </c:if>
                         <c:if test="${resource.state != 3 }">
                       	  <input class="ext_btn ext_btn_error" value="未审核" type="button" onclick="audit(this,${resource.id})" data-type="1"/>
                         </c:if>
                    </span> 
                </td>
            </tr>
...
var roleCode = "${sessionScope.baseUser.roleCode}";
 ...
 function PageTmp(name,courseId,size,page){
 	...
 	$.ajax({
		  type: "POST",
		  url: "${res}/resource/courseResList.html",
		  processData:true,
 ...
 function HtmlTmp(datas,htmlStr,startIndex){
 	...
 	$.each(datas, function (index, resource) {
			...
			htmlStr+="<input name='down_flies' class='down_flies' value='"+resource.url+"' type='hidden' id='"+resource.name+"'>";
...
//搜索条件查询列表
 ...
 $(".pagenxt").click(function(){
	 if(($(".current a").html()) + 1>$(".totalpage").val()){
		 layer.msg("已是最后一页！");
	 }else{
		 getPage(Number($(".current a").html()) + 1,$(".code").val(),$(".courseId").val());
	 }
 });
 ...
 $(".paginItem a").click(function(){
	 if(!isNaN(this.innerHTML))
	 	getPage(this.innerHTML,$(".code").val(),$(".courseId").val());
 });
 $(".scbtn").click(function(){
	 getPage(1,$(".input-text").val(),$(".select").val());
 });
 function getPage(page,code,courseId){
	 location.href="toResourceList1.html?pageNum="+page+"&&code="+code+"&&courseId="+courseId;
 }
 ...
 function audit(obj,pid){
	    ...
	    if(state=="-1"){
	        if(doAudit(pid,1)){//1表示分享
	           ...
	           $(obj).val("未审核").addClass("ext_btn_error").removeClass("ext_btn_success");
	        ...
	        if(doAudit(pid,-1)){//-1表示取消
	           $(obj).val("已审核").addClass("ext_btn_success").removeClass("ext_btn_error");
	           $(obj).attr("data-type","-1"); 
	...
	function doAudit(pid,type){
		...
		$.ajax({
			...
			type:"post",
			async: false,
			data:{fileId:pid,type:type,fileType:1},
	        		...
	        		success:function(data){
	        			if(1 == data.flag){
	    					  layer.alert(data.msg,{icon: 1, title:'提示'},function(){
	    	      				  document.location.href=" ${res}/resource/courseResList.html";
	    	      			  });
	...
	$("#start-btn").click(function(){
			...
			var resourceId = $(this).parent().parent().siblings(".resourceId").val();
			//提交点击量
			$.ajax({
				type: "GET",
				url:"${res}/resource/setClickNum.html",
				date:{resourceId:resourceId,fileType:1},
				success: function(data){
